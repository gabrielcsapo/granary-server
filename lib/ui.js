...
module.exports = function() {
    ...
    return {
                                    ...
                                    fs.stat(path.join(storage, folder, file), function(err, stat) {
                                        ...
                                        Project.getStats(_file).then(function(stats) {
                                            if(stats.bundle) {
                                                stats.details = stat;
                                                stats.name = file
                                                data.projects[folder].push(stats);
                                                data.count.files += 1;
                                            }
                                            counter -= 1;
                                            done();
        ...
        download: function(req, res) {
            if (req.params.file) {
                logger.debug('UI Bundle Download request', req.params.file);
        ...
        delete: function(req, res) {
            if (req.params.file) {
                logger.debug('UI Bundle Delete request', req.params.file);
                ...
                logger.info('UI Bundle Deleted:', req.params.file);
